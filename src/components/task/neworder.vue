	...
	<section>
		<div class="search-box">
			<div class="oprateSquare clear">
				<div class="orderTitle">{{basicinfoForm.name}}</div>
				<ul class="tabs">
				...
				<div class="fr">
					...
					<el-button size="small" type="text" @click="$router.push('/task/list')">取消</el-button>
				</div>
			</div>
		</div>
		<div class="bggrey" v-if="navs[0].active" :style="{'min-height':greybgheight+40+ 'px'}">
		...
		<div class="bggrey" v-if="navs[1].active" :style="{'min-height':greybgheight+40+ 'px'}">
			<div class="simple-stack">
				<div class="page">
					<div class="productinfo">
						<el-input class="big" v-model="newKno" clearable placeholder="请输入产品款号，按回车即可" @keyup.enter.native="addproductFn"></el-input>
						<ul>
							<li v-for="(item,index) in productlist" :key="index" @click="goproductFn(item)">{{item.kno}}</li>
						</ul>
					</div>
				</div>
			</div>
		</div>
		<div class="bggrey" v-if="navs[2].active" :style="{'height':greybgheight+40+ 'px'}">
			<div>
				<draggable v-model="taskslist" :options="{draggable:'.tasksblock'}">
					<transition-group>
						<div class="tasksblock" v-for="(item,index) in taskslist" :key="item.oid" :style="{'height':greybgheight+ 'px'}">
							<i class="iconfont icon-shanchu deletebig" @click="deletebigblockFn(taskslist,index)"></i>
							<h1>{{item.title}}
								...
								<draggable class="list-group" element="ul" v-model="item.tasks" :options="dragOptions" :move="onMove" @start="isDragging=true" @end="isDragging=false">
									<transition-group type="transition" :name="'flip-list'">
										<li class="commonblock" v-for="(itemT,indexT) in item.tasks" :key="itemT.order">
											<i class="iconfont icon-shanchu" @click="deleteblockFn(item.tasks,indexT)"></i>
											<div class="forclick">
		...
		<div class="bggrey" v-if="navs[3].active" :style="{'min-height':greybgheight+40+ 'px'}">
			<div class="simple-stack">
...
<script>
	...
	import { taskslist, chargepersonlist, basicinfo, productlist } from '@/datas/neworder'
	export default {
		name: 'neworder',
		components: {
			draggable,
		...
		methods: {
			...
			goproductFn(item) {
				sessionStorage.setItem('kno', item.kno);
				this.$router.push('/task/product/1');
			},
			...
			deleteblockFn(itemtask, indexT) {
				itemtask.splice(indexT, 1)
			},
			deletebigblockFn(taskslist, index) {
