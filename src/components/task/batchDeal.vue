	...
	<section>
		<div class="search-box" :class="{'h120':filtershow}">
			<div class="oprateSquare clear">
				...
				<div class="fr">
					<el-button size="small" type="success" @click="goDealingFn">完成选择</el-button>
		...
		<div class="bggrey batchpage" :style="{'height':greybgheight+40+ 'px'}">
			...
			<div class="tasksblock" v-for="(item,index) in taskslist" :key="index" :style="{'height':greybgheight+ 'px'}">
				<h1>{{item.title}}
				...
				<!--<EasyScrollbar :barOption="myBarOption">-->
					<div class="overflowy" :style="{'height':greybgheight-145+ 'px'}">
						<li class="commonblock" v-for="(itemT,indexT) in item.tasks" :key="indexT" @click="gobatchboxFn(item.tasks,itemT,indexT)">
							<div class="forclick">
								<div class="title">
									<el-checkbox disabled v-model="itemT.checked"></el-checkbox>
									<a href="javascript:;" class="tname">{{itemT.title}}</a>
...
<script>
	...
	import { groupname, taskslist, chargepersonlist, batchbox } from '@/datas/batchdeal'
	export default {
		name: 'list',
		components: {
			draggable,
...
//				item.newshow=false;
			...
			goDealingFn(){
				this.$router.push('/task/batchdealing');
			}
		},
		computed: {
			dragOptions() {
