	...
	<div class="simple-stack" v-loading="loading" style="margin:45px auto">
		...
		<div class="page page-root">
			...
			<div class="taskdetail">
				...
				<ul class="checklist">
					<li v-for="(item,index) in Inspection.checklist">
						<h3>{{item.title}}</h3>
						<div class="checkblock">
							<el-input placeholder="请输入文字说明" class="noborder" type="textarea"></el-input>
							<fileuploadCheck></fileuploadCheck>
						</div>

						<el-switch style="margin-right:35px;" @change="switchFn(item,index)" class="fr" v-model="item.pass" active-color="#1f9aa5" inactive-color="#c0c4cc" active-text="通过" inactive-text="不通过">
						</el-switch>
					...
					<li class="allcheck">
                           ...
                           <el-switch  @change="" class="allpass" v-model="allpass" active-color="#1f9aa5" inactive-color="#c0c4cc" active-text="通过" inactive-text="不通过">
						  </el-switch>
						   <label>这个任务检验是否通过？</label>
					</li>
				</ul>
				<div class="fileblock">
...
<script>
	...
	import fileuploadCheck from '@/components/common/fileuploadCheck'
	import { Inspection } from '@/datas/taskDetail2'
	export default {
		name: 'newTemplate',
		components: {
...
//						pass: false,
//					};
//					this.Inspection.checklist.push(ob);
				} else {
